...
import com.example.demo.core.utils.MessageTemplateUtil;
...
import com.example.demo.elastic.xmlbean.AddFields;
import com.example.demo.elastic.xmlbean.Convert;
...
//    protected String routingValue = "";
    ...
    private static final Logger log = LoggerFactory.getLogger(BaseMapper1.class);

    public BaseMapper1(ElasticTypeEnum typeEnum) throws Exception {
    ...
    public BaseMapper1(ElasticTypeEnum typeEnum, Map<String, Object> object) throws Exception {
        sourceObject= object;
        this.typeName = typeEnum.getEsType();
        this.fileName = typeEnum.getFileName();
        this.xmlMappers = XmlMapperUtil.getElasticMap(fileName);
    ...
    public ESBulkModel mapper(Map<String, Object> sourceData) throws Exception {
        ...
        if(!onMapper || xmlMappers.size() == 0){    //是否启用mapper开关, 关闭时直接copy 整个数组
            ...
            // 获取线程池
            ThreadPoolTaskExecutor convertThreadPool = SpringUtils.getBean("convertThreadPool");
            List<Future> futureList = new ArrayList<>();
            for(int i=0; i< xmlMapperSize; i++){
                XmlMapper map = xmlMappers.get(i);
                if(map.isIdField){
                    targetObject.put("id", this.sourceObject.get(map.getSourceName()));
                }
                if(map.isRoutingField){
                    targetObject.put("routing", this.sourceObject.get(map.getSourceName()));
                }
                if(map.isParentField){
                    targetObject.put("parent", this.sourceObject.get(map.getSourceName()));
                }
                ...
                if(!map.convert.getConvert()){  //不需要对值进行转换
                    ...
                    Future<Boolean> future = convertThreadPool.submit(callable);
    ...
    final protected void convertField(XmlMapper map) throws Exception {
        ...
        String sourceName = map.getSourceName();
        if(curConvertType.equals(ElasticMapperXmlEnum.PROPERTY_METHOD.getName())){  //通过SQL获取转换值
            Object v = doMethod(map.convert, map.getSourceName());
            targetObject.put(targetName, v);

        }else if(curConvertType.equals(ElasticMapperXmlEnum.PROPERTY_SQL.getName())){  // 通过方法执行
