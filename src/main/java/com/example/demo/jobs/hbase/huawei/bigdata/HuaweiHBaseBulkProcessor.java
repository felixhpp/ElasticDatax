 ...
 * @author felix
 */
public class HuaweiHBaseBulkProcessor {
    private static Logger logger = LoggerFactory.getLogger(HuaweiHBaseBulkProcessor.class);
    private static HuaweiHBaseBulkProcessor processor;
    ...
    private HuaweiHBaseBulkProcessor(Configuration conf ) {
        try {
            // 执行华为登录
            HuaweiHBaseUtil.login(conf);
            this.conn = ConnectionFactory.createConnection(conf);

        } catch (IOException e) {
            logger.error("Failed to login because ", e);
            return;
        }
     ...
     */
    public static HuaweiHBaseBulkProcessor getInstance(Configuration conf) {
        if (null == processor) {
            // 多线程同步
            synchronized (com.example.demo.jobs.hbase.HBaseBulkProcessor.class) {
                if (null == processor) {
                    processor = new HuaweiHBaseBulkProcessor(conf);
                }
            }
        }
